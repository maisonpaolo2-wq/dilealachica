...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!w) return {};
  return { title: `${w.couple} / ${w.place}`, description: `Boda diseñada y coordinada por Dile a la Chica. ${w.place}, ${w.year}.` };
}

const gallery = [
  { src: "/photos/p6.jpg", alt: "Momento íntimo", ratio: "4/5" },
  { src: "/photos/p7.jpg", alt: "Celebración", ratio: "1/1" },
    ...
    <main>
      ...
      <section style={{ background: "var(--cream)", padding: "clamp(64px,8vw,110px) clamp(20px,6vw,88px)", textAlign: "center" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <span className="eyebrow" style={{ display: "block", marginBottom: 16 }}>Su día</span>
          <p className="serif" style={{ fontSize: "clamp(22px,2.6vw,32px)", fontWeight: 400, fontStyle: "italic", color: "var(--ink)", lineHeight: 1.5, margin: "0 0 20px" }}>Una celebración diseñada y coordinada de principio a fin en {w.place.toLowerCase()}.</p>
          <p className="sans" style={{ fontSize: 15, color: "var(--ink-soft)", lineHeight: 1.85, margin: 0 }}>La historia completa y galería se añadirán con permiso de la pareja.</p>
        </div>
      </section>
      ...
      <section style={{ background: "var(--cream-2)", padding: "0 clamp(20px,6vw,88px) clamp(64px,8vw,110px)" }}>
        <div className="wrap">
          ...
          <div style={{ marginTop: 44, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
            ...
            <Link href="/contacto" className="btn btn-coral">Quiero la mía</Link>
          </div>
        </div>
      </section>
    </main>
